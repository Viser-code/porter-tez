    ...
    <section id="pricing" className="bg-gray-50 py-10 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 text-center">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-gray-900 mt-2">Тарифы</h2>
          <p className="text-gray-500 mt-3 max-w-md mx-auto">
            Фиксированная цена на популярные услуги. Для крупных заказов — индивидуальный расчёт.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-6">
          {plans.map(({ name, sub, price, unit, highlight, features }) => (
            <div
              ...
              <h3
                className={`text-xl font-black mb-1 ${highlight ? 'text-white' : 'text-gray-900'}`}>
                {name}
              </h3>
              ...
              <div
                ...
                <span className={`text-sm ${highlight ? 'text-orange-100' : 'text-gray-400'}`}>
                  {unit}
                </span>
              </div>
              ...
              <ul className="flex-1 space-y-3 mb-8">
                {features.map(f => (
                  <li key={f} className="flex items-start gap-2">
                    ...
                    <span className={`text-sm ${highlight ? 'text-orange-50' : 'text-gray-600'}`}>
                      {f}
                    </span>
                  </li>
                ))}
